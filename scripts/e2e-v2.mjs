...
assert('详情区块齐全（等级白字90/潜能/装备/精英化/技能）', detail.includes('当前等级') && detail.includes('LIMIT 90') && detail.includes('潜能') && detail.includes('装备与基质') && detail.includes('精英化') && detail.includes('天赋技能'), '')
...
const wpnSel = pg.locator('select').filter({ has: pg.locator('option:has-text("点心时刻")') }).first()
await wpnSel.selectOption('点心时刻')
await pg.waitForTimeout(400)
const d3 = await pg.evaluate(() => JSON.parse(localStorage.getItem('ef-roster') || '{}')['伊冯'].wpn.name)
assert('武器下拉保存', d3 === '点心时刻', d3)
