...
assert('练度三格可见（等级/晋升/潜能）', detail.includes('当前等级') && detail.includes('晋升') && detail.includes('潜能'), '')
...
const dia = pg.locator('.diamonds').first().locator('span').nth(2)
await dia.click()
await pg.waitForTimeout(400)
const d2 = await pg.evaluate(() => JSON.parse(localStorage.getItem('ef-roster') || '{}')['伊冯'].promo)
assert('晋升 → 3', d2 === 3, String(d2))
await pg.locator('button:has-text("武器与装备")').click()
await pg.waitForTimeout(500)
