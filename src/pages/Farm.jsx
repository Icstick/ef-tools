...
export default function FarmPage() {
  ...
  const cell = { background: 'var(--paper)', border: '1px solid var(--line)', color: 'var(--ink)', borderRadius: 'var(--radius)', padding: '4px 8px', fontSize: 13 }
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
        ...
        <button onClick={push} style={{ ...cell, cursor: 'pointer', color: 'var(--yellow)', fontWeight: 700 }}>添加</button>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {rows.length === 0 && <div style={{ color: 'var(--sub)', padding: 20, textAlign: 'center' }}>还没有记录——添加你要刷的副本/地点（练度/材料数据请手动记录，后续接入自动数据源）</div>}
        {rows.map(r => (
          <div key={r.id} style={{ display: 'flex', gap: 8, alignItems: 'center', borderBottom: '1px solid var(--line)', padding: '6px 2px' }}>
            ...
            <span style={{ color: 'var(--sub)', fontSize: 12 }}>{r.date}</span>
            <button onClick={() => del(r.id)} style={{ ...cell, cursor: 'pointer', color: '#a33', padding: '1px 8px' }}>删</button>
          </div>
        ))}
      </div>
    </div>
